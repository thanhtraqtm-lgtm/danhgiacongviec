...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const getRoleDisplayName = () => {
    ...
    if (globalRole === 'DEPT_HEAD') return 'Trưởng phòng';
    return 'Chuyên viên';
  };
    ...
    <div className="w-72 bg-[#005C35] text-white fixed top-0 left-0 bottom-0 flex flex-col z-40 border-r border-[#004A2A] shadow-md font-sans">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg transition-all duration-150 text-[12.5px] ${
                          isChildActive
                            ? 'bg-yellow-300 text-[#004A2A] font-semibold shadow-xs'
                            : 'text-white/85 hover:bg-[#006642] hover:text-white font-normal'
                        }`}
                      >
                        <span className={`shrink-0 ${isChildActive ? 'text-[#004A2A]' : 'text-white/70'}`}>
                          {child.icon}
                        </span>
      ...
      <div className="p-3 border-t border-[#003D22] bg-[#004A2A] shrink-0">
        <div className="flex items-center justify-between gap-2.5 px-3 py-2 bg-white/5 rounded-xl border border-white/10">
          <div className="w-8 h-8 rounded-lg bg-red-600 text-white font-bold flex items-center justify-center text-xs shadow-xs shrink-0 ring-1 ring-white/20">
            {currentUser ? (currentUser.fullName ? currentUser.fullName.charAt(0) : 'U') : 'AD'}
          </div>
          <div className="min-w-0 flex-1 text-center">
            <div className="text-xs font-bold text-white truncate" title={currentUser ? currentUser.fullName : 'Quản trị viên'}>
          ...
          {currentUser && onLogout ? (
            <button
              type="button"
              onClick={onLogout}
              className="p-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/40 text-rose-300 hover:text-white transition-colors shrink-0"
              title="Đăng xuất"
            >
